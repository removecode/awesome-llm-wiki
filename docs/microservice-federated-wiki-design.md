...
业务域需要像服务一样有稳定的结构化对象和编译产物，不能在查询时临时拼出一份无版本的概览：

```text
domain/<domain_id>/
├── manifest.yaml
├── domain.yaml          # 域名称、确认状态、服务成员及 primary/supporting/shared 角色
└── wiki/
    ├── overview.md      # 业务域职责、边界和核心能力
    ├── services.md      # 服务清单、角色及引用的 service section
    └── flows.md         # 已确认关键流程；候选路径单独分区并显式标记
```

`domain.yaml` 保存负责人确认过的域名称和成员关系，也可保存算法候选标签，但两者必须使用不同状态字段，
禁止把社区发现结果静默提升为事实。`wiki/` 由这些结构化关系和已确认 flow 编译而来，只引用服务知识，
不复制单服务的接口或架构说明。
...
流程同样需要明确的机器可读对象和人可读编译结果：

```text
flow/<flow_id>/
├── manifest.yaml
├── flow.yaml            # 状态、步骤、分支条件、入口/输出和证据
└── wiki/
    ├── overview.md      # 目标、边界、参与服务和整体说明
    └── steps.md         # 已确认步骤及其 service/endpoint 引用
```

`flow.yaml` 必须用 `status: candidate|confirmed` 区分调用图候选路径与负责人确认的业务流程。候选路径可以
参与发现和人工复核，但不得混入 `wiki/steps.md` 的已确认步骤，也不得被 `get_flow` 当作确定流程返回。
...
- **业务线整体认知**（“支付业务线包含哪些服务、整体怎么运转”）：K3 domain + 已确认 flow；C2–C3；
  D0–D2；`get_domain_context` 先返回域概览、带角色的服务清单和关键流程摘要，再按用户选择对单个 flow
  调用 `get_flow`。候选域标签、候选路径与已确认内容必须分开展示。
...
- **内部 SDK/公共能力主动加载**（代码新增或修改 import/依赖声明）：K3 `shared_capability` + 该能力的
  K2 用法与常见坑；D1–D2；由 7.6 节的 Hook 在编辑成功后触发，不等待新的自然语言问题。只加载本次
  新增或发生变化的依赖，并按能力 ID 和版本在会话内去重。
...
自然语言问题不是唯一的知识加载信号。编码过程中新增 import、包引用或依赖声明，已经能说明 Claude Code
即将使用某个内部 SDK/公共能力；如果仍要求工程师再问一次“这个 SDK 怎么用”，会错过最需要规范的时点。
因此主动加载由项目级 Hook 触发，MCP 方法仍保留为被动查询接口：

1. Hook 使用 `postToolUse` 监听代码编辑工具成功事件，从本次变更中提取新增或发生变化的 import、包名和
   依赖声明；不扫描未变化的全部依赖。
2. Hook 使用 K4 别名索引把依赖名解析为 `shared_capability`。只有确定命中内部能力时才继续，第三方包
   和多候选结果不自动注入。
3. Hook 通过与 MCP 共用的只读检索适配器，获取该能力 K2 中有界的用法、接口约束和常见坑，并以
   `additional_context` 注入当前代理上下文；不修改用户提示词，也不直接改代码。
4. 注入内容必须带能力 ID、section 引用、新鲜度和置信度。Claude Code 仍需在写调用代码前用 K1/K0
   核实具体版本的接口签名，Hook 提供的说明不能替代证据。
5. 同一会话按 `capability_id + content_hash` 去重；只有知识内容或依赖版本变化时才重新注入，避免每次
   保存文件都重复消耗上下文预算。

Hook 采用 fail-open：解析或检索失败不能阻塞代码编辑，但必须追加“未找到该依赖的内部使用规范”或
“候选不唯一”的提示，禁止根据包名猜测用法。Hook 与 MCP 使用相同的 ACL、可见范围和 token 预算；
它只是新增“代码变化 → 检索”的触发适配器，不创建第二份知识索引，也不绕过 7.4 节的上下文组装规则。
...
- `get_domain_context(domain_id)`：返回域概览、按 `primary/supporting/shared` 标注的服务清单和已确认
  关键流程摘要；候选域标签与候选路径必须单独返回，不得混入已确认内容。
